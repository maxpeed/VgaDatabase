...
use \VgaDatabase\VgaDatabaseException;
use \PDO;
use \PDOException;
use \PDOStatement;
...
class VgaDBC extends PDO
{
     ...
     *
     */
    function __construct(string $database, string $user, string $password, string $host = "localhost", array $opts = [])
    {
        ...
        $this->host = $host;
        $this->charset = isset($opts['charset']) ? $opts['charset'] : "utf-8";

        $this->opts = [
            PDO::ATTR_PERSISTENT => isset($opts['persistent']) ? $opts['persistent'] : FALSE,
            PDO::ATTR_ERRMODE => isset($opts['errmode']) ? $opts['errmode'] : PDO::ERRMODE_SILENT,
        ];
    }
